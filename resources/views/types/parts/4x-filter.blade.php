<div class="py-2">
    ...
    <form action="">
        ...
        <div class="form-check">
            ...
            <label class="form-check-label" for="{{$item->name}}">{{ ucfirst(__('custom.products.'.$item->name)) }}</label>
        </div>
        @endforeach
        @endif
    </form>
        ...
        $('.axles-filter').on('change', function() {
            if( $(this).is(':checked') ){
                ...
                if(config_axles.length == 0){
                    vehicles.forEach ( element => {
                        if(filters.includes(element.brand.toLowerCase())||filters.length == 0 && (cargo_type.includes(element.tank_trailer.type.name.toLowerCase()) || cargo_type.length==0) ){$("#"+element.id).fadeIn("slow");} //else if (filters.length == 0){$("#"+element.id).fadeIn("slow");}
                    });
                }
            }
        });
        ...
        $('.tank-filter').on('change', function() {
            if( $(this).is(':checked') ){
                ...
                vehicles.forEach( element => {
                    ...
                    if(cargo_type.includes(element.tank_trailer.type.name.toLowerCase()) && (config_axles.includes(element.config_axles.toLowerCase()) || config_axles.length==0) ){
                        $("#"+element.id).show();console.log(element);
                        console.log('ok');
                    } else {
                        console.log('ko');
